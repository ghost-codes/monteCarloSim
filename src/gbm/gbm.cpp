...
GBM::GBM(double T, double N, double currentPrice, double vol, double riskFree, double yield)
    : d_dt(T / N), d_volatility(vol), d_riskFree(riskFree), d_dividendYield(yield), d_N(N), d_values(), d_initPrice(currentPrice)
{
}
...
{
    double z = computeZ();
    double exponentialDrift = d_riskFree - d_dividendYield - (pow(d_dividendYield, 2) / 2);
